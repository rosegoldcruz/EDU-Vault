...
function statusClasses(status: string): string {
  switch (status) {
    ...
    case 'queued': return 'text-muted border-line-strong bg-ink-raised'
    case 'processing': return 'text-muted border-line-strong bg-ink-raised'
    case 'paid': return 'text-acid border-acid/30 bg-acid/10'
    case 'failed': return 'text-[#ffb4a8] border-[#6f3934] bg-[#6f3934]/15'
    case 'canceled': return 'text-muted border-line bg-ink-raised'
    default: return 'text-muted border-line bg-ink-raised'
  }
}
...
export function AdminRewardsDashboard() {
  ...
  return (
    <section className="space-y-6">
      ...
      {error ? <div className="border border-[#6f3934] bg-ink-soft p-5 text-sm text-[#ffb4a8]">{error}</div> : null}
      ...
      {summary ? (
        <div className="iv-panel p-5">
          ...
          <p className="text-sm text-acid break-all">{summary.totalAmountRawPaid}</p>
        </div>
      ) : null}
      ...
      <div className="iv-panel p-5">
        ...
        <div className="space-y-3">
          {filteredJobs.length === 0 ? <p className="text-sm text-muted">No jobs found.</p> : null}
          {filteredJobs.map((job) => (
            <div key={job.id} className="border border-line bg-ink p-4">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div className="min-w-0">
                  <p className="iv-card-title text-xl break-all">{job.id}</p>
                  <p className="text-xs text-muted break-all">User: {job.privy_user_id}</p>
                  <p className="text-xs text-muted break-all">Wallet: {job.wallet_address}</p>
                  <p className="text-xs text-muted">Milestone {job.milestone_number} · Amount {job.amount_raw}</p>
                  <p className="text-xs text-muted">Attempts {job.attempts} / {job.max_attempts}</p>
                ...
                <div className="flex flex-col items-end gap-2">
                  ...
                  {job.status === 'failed' ? (
                    <button
                      ...
                      className="border border-acid/40 bg-acid/10 px-3 py-1.5 text-xs text-acid disabled:opacity-50"
                    >
      ...
      <div className="iv-panel p-5">
        ...
        <div className="space-y-3">
          {transactions.length === 0 ? <p className="text-sm text-muted">No transactions found.</p> : null}
          {transactions.map((tx) => (
            <div key={tx.id} className="border border-line bg-ink p-4">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div className="min-w-0">
                  <p className="iv-card-title text-xl">Milestone {tx.milestone_number}</p>
                  <p className="text-xs text-muted break-all">Signature: {tx.signature}</p>
                  <p className="text-xs text-muted break-all">User: {tx.privy_user_id}</p>
                  <p className="text-xs text-muted">Amount Raw: {tx.amount_raw}</p>
                  <p className="text-xs text-muted">Confirmed: {tx.confirmed_at ?? 'Pending'}</p>
                </div>
