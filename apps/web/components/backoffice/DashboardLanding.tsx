        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {[
            ...
            <div key={item.label} className="iv-panel p-4">
              ...
              <p className="font-mono text-sm text-white break-all">{item.value}</p>
            </div>
          ))}
        </div>
      </div>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {quickLinks.map((card) => {
            ...
            return (
              ...
              >
                <div className={`mb-4 inline-flex h-11 w-11 items-center justify-center border ${isLime ? 'iv-chip-lime' : 'iv-chip-purple'}`}>
                  <Icon className="h-5 w-5" />
                </div>
                <h3 className="iv-card-title mb-2 text-2xl">{card.title}</h3>
                <p className="iv-body mb-4 text-sm">{card.desc}</p>
                <span className="inline-flex items-center gap-2 font-mono text-[10px] uppercase tracking-[0.16em] text-acid">
