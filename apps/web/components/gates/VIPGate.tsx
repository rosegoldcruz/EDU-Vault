  ...
  if (loading) {
    return (
      <div className="iv-panel grid min-h-[520px] place-items-center p-8">
        <div className="h-12 w-12 animate-pulse rounded-full border border-acid/30 bg-acid/10" />
      </div>
    )
  }
    ...
    <section className="relative min-h-[calc(100vh-8rem)] overflow-hidden border border-line bg-ink">
      ...
      <div className="relative z-10 grid min-h-[calc(100vh-8rem)] place-items-center px-4 py-10">
        ...
        <form onSubmit={handleSubmit} className={`relative z-20 w-full max-w-md border border-line bg-ink/90 p-6 shadow-[0_0_70px_rgba(86,230,40,0.10)] transition duration-500 sm:p-8 ${opening ? 'scale-95 opacity-0' : 'scale-100 opacity-100'}`}>
          ...
          {!configured ? (
            <div className="border border-[#6f3934] bg-[#6f3934]/15 p-4 text-sm text-[#ffb4a8]">
      ...
      <style jsx>{`
        ...
        .vip-vault-door {
          ...
          width: 50%;
          background:
            radial-gradient(circle at center, rgba(86, 230, 40, 0.12), transparent 28%),
            repeating-linear-gradient(90deg, #291845 0 18px, #311e52 18px 36px);
          border-color: rgba(86, 230, 40, 0.18);
          box-shadow: inset 0 0 60px rgba(8, 11, 15, 0.9);
        ...
        .vip-vault-door-left {
          ...
          border-right: 1px solid rgba(86, 230, 40, 0.28);
        }

        .vip-vault-door-right {
          right: 0;
          transform-origin: right center;
          border-left: 1px solid rgba(86, 230, 40, 0.28);
        }
        ...
        .vip-vault-ribs {
          position: absolute;
          inset: 22px;
          border: 1px solid rgba(86, 230, 40, 0.14);
          background: repeating-linear-gradient(0deg, transparent 0 34px, rgba(86, 230, 40, 0.08) 34px 36px);
        }
        ...
        .vip-vault-lock {
          ...
          border: 1px solid rgba(86, 230, 40, 0.35);
          border-radius: 999px;
          background: #291845;
          box-shadow: 0 0 54px rgba(86, 230, 40, 0.18), inset 0 0 32px rgba(8, 11, 15, 0.9);
