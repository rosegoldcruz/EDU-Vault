    ...
    <section className="space-y-6">
      <div className="iv-panel iv-panel-lime p-6 sm:p-8">
        ...
        <div className="grid gap-4 lg:grid-cols-[1.25fr_0.75fr]">
          <div className="iv-panel p-5 sm:p-6">
            ...
            <p className="iv-card-title text-5xl text-acid">{profile?.referral_code ?? 'Unavailable'}</p>
            <div className="mt-4 border border-line bg-ink/45 p-4">
              <p className="iv-label-muted mb-2">Referral link</p>
              <p className="break-all font-mono text-sm leading-6 text-white">{referralLink || 'Referral link available after profile sync.'}</p>
            </div>
          ...
          <div className="iv-panel p-5 sm:p-6">
            ...
            <p className="iv-title text-7xl text-acid">{referrals.length}</p>
      ...
      <div className="grid gap-6 2xl:grid-cols-[1.05fr_0.95fr]">
        ...
        <div className="iv-panel p-6 sm:p-8">
          ...
          {referrals.length === 0 ? <p className="text-sm text-muted">No referral leads submitted yet.</p> : (
            <div className="space-y-3">
              {referrals.map((lead) => (
                <article key={lead.id} className="border border-line bg-ink/90 p-5">
                  <div className="flex items-start justify-between gap-3">
                    <div><p className="iv-card-title text-2xl">{lead.name}</p><p className="text-sm text-muted">{lead.phone}</p></div>
                    <span className="border border-line px-2 py-0.5 font-mono text-[10px] uppercase tracking-[0.14em] text-muted">{lead.status}</span>
                  </div>
